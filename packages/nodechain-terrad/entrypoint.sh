...
terrad init "terra-nodechain" --chain-id ${NETWORK}

if [[ -n "${TESTNET}" ]]; then
  wget https://raw.githubusercontent.com/terra-money/testnet/master/${NETWORK}/genesis.json && cp genesis.json ~/.terra/config/genesis.json
  wget https://raw.githubusercontent.com/terra-money/testnet/master/${NETWORK}/addrbook.json -O ~/.terra/config/addrbook.json
