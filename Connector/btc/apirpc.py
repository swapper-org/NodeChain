...
def getAddressHistory(id, params):
    ...
    addrHistory = RPCConnector.request(
        RPC_ELECTRUM_ENDPOINT, id, GET_ADDRESS_HISTORY_METHOD, [params[ADDRESS]])
...
def getAddressBalance(id, params):
    ...
    connResponse = RPCConnector.request(
        RPC_ELECTRUM_ENDPOINT, id, GET_ADDRESS_BALANCE_METHOD, [params[ADDRESS]])
...
def getAddressUnspent(id, params):
    ...
    connResponse = RPCConnector.request(
        RPC_ELECTRUM_ENDPOINT, id, GET_ADDRESS_UNSPENT_METHOD, [params[ADDRESS]])
...
def getBlockByNumber(id, params):
    ...
    blockHash = RPCConnector.request(
        RPC_CORE_ENDPOINT, id, GET_BLOCK_HASH_METHOD, [blockNumber])
...
def getFeePerByte(id, params):
    ...
    feePerByte = RPCConnector.request(
        RPC_CORE_ENDPOINT, id, ESTIMATE_SMART_FEE_METHOD, [confirmations])
...
def getHeight(id, params):
    ...
    latestBlockHeight = int(RPCConnector.request(
        RPC_CORE_ENDPOINT, id, GET_BLOCK_COUNT_METHOD, []))
    latestBlockHash = RPCConnector.request(
        RPC_CORE_ENDPOINT, id, GET_BLOCK_HASH_METHOD, [latestBlockHeight])
...
def getTransactionHex(id, params):
    ...
    rawTransaction = RPCConnector.request(
        RPC_ELECTRUM_ENDPOINT, id, GET_TRANSACTION_METHOD, [params[TX_HASH]])
...
def getTransaction(id, params):
    ...
    transactionRaw = RPCConnector.request(
        RPC_ELECTRUM_ENDPOINT, id, GET_TRANSACTION_METHOD, [params[TX_HASH]])
    transaction = RPCConnector.request(
        RPC_CORE_ENDPOINT, id, DECODE_RAW_TRANSACTION_METHOD, [transactionRaw])
...
def getTransactionCount(id, params):
    ...
    response = {
        TRANSACTION_COUNT: str(
            pending) if params[PENDING] else str(len(txs) - pending)
    }
...
def notify(id, params):
    ...
    payload = RPCConnector.request(RPC_ELECTRUM_ENDPOINT, id, NOTIFY_METHOD, [
